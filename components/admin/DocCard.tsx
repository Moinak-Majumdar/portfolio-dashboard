...
interface props {
  darkMode: boolean,
  data: { _id: string, name: string, type: string, status: string, role: string, intro: string, liveUrl: string, gitRepo: string, slug: string, description: string, img: string[], tools: string[], toolsLogo: string[], cover: string}
}
const DocCard = ({ data, darkMode }:props) => {
  return (
    <Link href={`admin/${data.name}`} className='flex flex-col w-72 rounded-lg overflow-hidden h-fit relative cursor-pointer' >
      ...
      <Image src={data.cover} width='288' height='220' className='transition transform-gpu duration-700 ease-in-out hover:scale-105' alt='image.png' />
