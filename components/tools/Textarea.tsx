...
const Textarea = ({id, autoComplete, onChange, value, name, required, type, disable, darkMode, theme, placeholder, className}) => {
    ...
    return (
        <textarea 
            ...
            className={`w-full h-24 resize-y rounded border outline-none py-1 px-3 leading-6 transition-colors duration-200 ease-in-out  ${darkMode ? 'bg-gray-800 border-gray-700 text-gray-100' : 'text-gray-700   border-gray-300 bg-white'} ${disable ? 'cursor-not-allowed': ''} ${className}`}
